...
/*   Updated: 2021/12/06 12:01:06 by jpceia           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		bool fork_builtin, int fd_in, int fd_out)
{
	...
	status = command_tree_execute_io(left, fork_builtin, fd_in, fd_out);
	if (status == 0)
		status = command_tree_execute_io(right, fork_builtin, fd_in, fd_out);
	return (status);
}
